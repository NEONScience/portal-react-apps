...
COPY --from=builder-parent /usr/src/app/build-stage/portal-react-apps/apps/taxonomic-lists/node_module[s] \
  /usr/src/app/build-temp/portal-react-apps/apps/taxonomic-lists/node_modules

# Build the taxonomic-lists app
RUN cd /usr/src/app/build-temp/portal-react-apps/apps/taxonomic-lists \
  && yarn run build
# Move build to working directory
RUN mv /usr/src/app/build-temp/portal-react-apps/apps/taxonomic-lists/build /usr/src/app/
...
ENTRYPOINT exec ./server \
  ...
  -app-cookie-jwt=$PORTAL_COOKIE_JWT \
