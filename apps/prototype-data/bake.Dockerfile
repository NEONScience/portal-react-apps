...
COPY --from=builder-parent /usr/src/app/build-stage/portal-react-apps/apps/prototype-data/node_module[s] \
  /usr/src/app/build-temp/portal-react-apps/apps/prototype-data/node_modules

# Install dependencies and build app
RUN cd /usr/src/app/build-temp/portal-react-apps/apps/prototype-data \
  && yarn run build
# Move build to working directory
RUN mv /usr/src/app/build-temp/portal-react-apps/apps/prototype-data/build /usr/src/app/
...
ENV PORTAL_CLIENT_ROUTE="/prototype-datasets"
ENV PORTAL_WEB_SERVER_METADATA_TYPE="PrototypeDataset"
